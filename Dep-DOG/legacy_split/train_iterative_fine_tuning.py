...
parser.add_argument('--suffix', type=str, default='round3_FT', help='Current Round Suffix') 
...
parser.add_argument('--prev_round_ckpt', type=str, default='out/MDMx0/cd4$_3_12_384_round2_1000000/1000000_ckpt.pt', help='Path to the checkpoint from the previous round to warm-start from')
...
def get_batch(split):
    ...
    if zero_mask_rows.any().item():
        raise RuntimeError(
            f"Dep-DOG/train_iterative_fine_tuning.py get_batch produced {zero_mask_rows.sum().item()} sample(s) without any masked response tokens; loss normalization would divide by zero."
        )
...
if init_from == 'scratch':
    ...
    gptconf = GPTConfig(**model_args)
    model = GPT(gptconf)
    # ... (the remaining weight-loading code stays the same) ...
    state_dict = checkpoint['model']
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        # Always overwrite 'ckpt_latest.pt' to support resume
    ...
    if iter_num >= max_iters:
        if master_process:
            ...
            # 2. Save as "{max_iters}_ckpt.pt" to match the Bash script
            ...
            # [Note] Do not update ckpt_latest.pt here so it remains the second-to-last archive
