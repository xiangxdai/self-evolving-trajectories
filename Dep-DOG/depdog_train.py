...
# =============================================================================
parser = argparse.ArgumentParser(description='Unified training of NanoGPT for Dep-DOG (all rounds)')
...
parser.add_argument('--out_dir', type=str, default=None, help='Base output directory (default: out/depdog_train)')
...
if args.out_dir is None:
    base_out_dir = REPO_ROOT / 'out' / 'depdog_train'
else:
...
min_lr = learning_rate / 10  # The minimum LR also decreases together with the peak LR
dtype = 'bfloat16'
...
def get_batch(split):
    ...
    if zero_mask_rows.any().item():
        raise RuntimeError(
            f"Dep-DOG/depdog_train.py get_batch produced {zero_mask_rows.sum().item()} sample(s) without any masked response tokens; loss normalization would divide by zero."
        )
...
def generate_data(model):
    ...
    # Look up the PAD token ID dynamically if it exists in the vocabulary
    pad_token_id = stoi.get('<PAD>', None)
    if pad_token_id is not None:
        print(f"[Info] '<PAD>' token detected (ID: {pad_token_id}). Enforcing 'Content First, PAD Last'.")
    
    # --- Choose the data source and block size based on the current round ---
    if round_num == 1:
    ...
    for i, start_idx in enumerate(total_range):
        ...
        for step in range(response_size):
            ...
            # 1. Extract the predicted probability of the ground-truth token
            p = probs.gather(dim=2, index=z_clean.unsqueeze(-1)).squeeze(-1)
            
            # 2. [Core improvement] Content First, PAD Last penalty logic
            if pad_token_id is not None:
                is_pad = (z_clean == pad_token_id)
                # Force undecoded PAD token confidence down to -0.5 so it stays below valid tokens but above already-decoded positions at -1.0
                p = torch.where(is_pad, torch.tensor(-0.5, device=device), p)
            
            # 3. Set already-decoded token confidence to -1.0 so it is never selected again
